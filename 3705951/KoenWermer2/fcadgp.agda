...
module Regular2Multirec where
  ...
  toμRegular C (Multirec.⟨_⟩ x) = Regular.⟨_⟩ (toRegular C (Multirec.map (r2mC C) (const (toμRegular C)) tt x))
  ...
  iso₁ (Regular._⊗_ F G)  (x , y)  = cong₂ _,_ (iso₁ F x) (iso₁ G y)
  lemma₁ : {R₁ R₂ : Set} (C : Regular.Code)
           {f : R₁ → R₂} (x : Multirec.⟦_⟧ (r2mC C) (const R₁) tt)
         → toRegular C (Multirec.map (r2mC C) (const f) tt x)
         ≡ Regular.map C f (toRegular C x)
  lemma₁ Regular.U     _ = refl
  lemma₁ Regular.I'    _ = refl
  lemma₁ (Regular._⊕_ F G) (inj₁ x) = cong inj₁ (lemma₁ F x)
