...
normalizableStep : forall {ty} -> {c1 c2 : Closed ty} -> Step c1 c2 -> Normalizable ty c2 -> Normalizable ty c1
normalizableStep {O} s (Halts nf s') = Halts nf (Cons s s')
normalizableStep {ty => ty₁} s (Halts nf s' , a) = Halts nf (Cons s s') , (λ c n → normalizableStep (AppL _ _ c s) (a c n))
