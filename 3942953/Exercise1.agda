...
vmap x Nil = Nil
vmap x (Cons y ys) = Cons (x y) (vmap x ys)
...
vadd Nil Nil = Nil
vadd (Cons x x₁) (Cons y y₁) = Cons (x + y) (vadd x₁ y₁)
...
idMatrix {Zero} = Nil
idMatrix {Succ n} = Cons (Cons 1 (pure {n} 0)) (vmap (Cons 0) (idMatrix {n}))

-- Define matrix transposition
transpose : {n m : Nat} {a : Set} -> Matrix a m n -> Matrix a n m
transpose {Zero} x = pure Nil
transpose {Succ n} (Cons x xs) = (vmap (λ a → λ b → Cons a b) x) <*> (transpose xs)
...
cmp (Succ n) (Succ m) = helper n m (cmp n m)
  where
  helper : (n m : Nat) -> Compare n m -> Compare (Succ n) (Succ m)
  helper n₁ .(n₁ + Succ k) (LessThan k) = LessThan k
  helper n₁ .n₁ Equal = Equal
  helper .(m₁ + Succ k) m₁ (GreaterThan k) = GreaterThan k
...
difference (Succ n) (Succ m) = difference n m
...
-- Dit kan vast mooier
plusCommutes (Succ n) (Succ m) = cong Succ ((trans (trans (sym (plusSucc n m)) (cong Succ (plusCommutes n m))) (plusSucc m n)))
...
SubListTrans Base y = y
SubListTrans (Keep x) (Keep y) = Keep (SubListTrans x y)
SubListTrans (Drop x) (Keep y) = Drop (SubListTrans x y)
SubListTrans x (Drop y) = Drop (SubListTrans x y)

SubListAntiSym : {a : Set} {xs ys : List a} ->  SubList xs ys -> SubList ys xs -> xs == ys
SubListAntiSym {xs = Nil} {Nil} x y = Refl
SubListAntiSym {xs = Nil} {Cons x ys} (Drop x₁) ()
SubListAntiSym {xs = Cons x xs} {Nil} () y
SubListAntiSym {xs = Cons x xs} {Cons .x ys} (Keep slx) (Keep sly) = cong (Cons x) (SubListAntiSym slx sly)
SubListAntiSym {xs = Cons x xs} {Cons .x ys} (Keep slx) (Drop sly) = {!!}
SubListAntiSym {xs = Cons x xs} {Cons .x ys} (Drop slx) (Keep sly) = {!!}
SubListAntiSym {xs = Cons x xs} {Cons y ys} (Drop slx) (Drop sly) = {!!}
...
leqTrans Base (Step x₁) = Base
leqTrans (Step x) (Step x₁) = Step (leqTrans x x₁)

leqAntiSym : {n m : Nat} -> LEQ n m -> LEQ m n -> n == m
leqAntiSym Base Base = Refl
leqAntiSym (Step x) (Step x₁) = cong Succ (leqAntiSym x x₁)
...
<=leq Zero Zero x = Base
<=leq Zero (Succ m) x = Base
<=leq (Succ n) Zero ()
...
notNotExcludedMiddle x = x (Inr (λ z → x (Inl z)))
...
step1 dn = dn (λ z → z (Inr (λ x → z (Inl x)))) 

step2 : excludedMiddle -> impliesToOr
step2 em ptoq = orCase (λ x → Inr (ptoq x)) (λ x → Inl (x)) em

id : {P : Set} -> P -> P
id x = x

step3 : impliesToOr -> doubleNegation
step3  ito {P} h = orCase (λ (x : Not P) → (magic (h x))) id (ito id)

-- HARDER: show that these are equivalent to Pierces law:
piercesLaw = {P Q : Set} -> ((P -> Q) -> P) -> P

ltr : piercesLaw -> excludedMiddle
ltr z = z (λ y → Inr (λ x → y (Inl x)))

rtl : doubleNegation -> piercesLaw
rtl z y = z (λ x → x (y (λ w → z (λ _ → x w))))
