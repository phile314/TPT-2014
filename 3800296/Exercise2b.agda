...
normalizableStep {ty => ty₁} (AppL f f' x s) (Halts x₁ x₂ , x₃) = Halts x₁ (Cons (AppL f f' x s) x₂) ,
  (λ x₄ n → normalizableStep (AppL (Clapp f x) (Clapp f' x) x₄ (AppL f f' x s)) (x₃ x₄ n))
normalizableStep {ty => ty₁} (Beta body v) (Halts x x₁ , x₂) = Halts x (Cons (Beta body v) x₁) ,
  (λ x₃ n → normalizableStep (AppL (Clapp (Closure (Abs body) _) v) (Closure body (Cons v _)) x₃ (Beta body v)) (x₂ x₃ n))
normalizableStep {ty => ty₁} (Lookup {_}{._}{env}{i}) (Halts x x₁ , x₂) = Halts x (Cons Lookup x₁) ,
  (λ x₃ n → normalizableStep (AppL (Closure (Var env) i) (lookup env i) x₃ Lookup) (x₂ x₃ n))
normalizableStep {ty => ty₁} Dist (Halts x₁ x₂ , x₃) = Halts x₁ (Cons Dist x₂) ,
  (λ x n → normalizableStep (AppL (Closure (App _ _) _) (Clapp (Closure _ _) (Closure _ _)) x Dist) (x₃ x n))
...
mutual
  ...
  clapp-normalization (x , x₁) n2 = x₁ _ n2
  ...
  closure-normalization (App t t₁) env n with closure-normalization t env n | closure-normalization t₁ env n
  closure-normalization (App t t₁) env n | Halts x x₁ , x₂ | p' = normalizableStep Dist (x₂ (Closure t₁ env) p')
  closure-normalization (Var Top) (Cons x env₁) (x₁ , x₂) = normalizableStep Lookup x₁
  closure-normalization {v :: G} (Var (Pop x)) (Cons t env) (x₂ , x₃) = closure-normalization (Var (Pop x)) (Cons t env) (x₂ , x₃)
  closure-normalization {[]} (Abs t) Nil U = {!!} , {!!}  
  closure-normalization {O :: ctx} {u => v} (Abs t) (Cons x env) (Halts x₁ x₂ , x₃) = {!!}
  closure-normalization {(c => c₁) :: ctx} {u => v} (Abs t) (Cons x env) ((x₁ , x₂) , x₃) = {!!}
  ...
  normalizable-env {._} {Cons x env} = normalization x , normalizable-env

  -- All terms are normalizable.
  normalization : {ty : Type} -> (c : Closed ty) -> Normalizable ty c
  normalization (Closure t Nil) = closure-normalization t Nil U
  normalization (Closure t (Cons x env)) = closure-normalization t (Cons x env) (normalization x , normalizable-env)
  normalization (Clapp c c₁) = clapp-normalization (normalization c) (normalization c₁)

termination : (ty : Type) -> (c : Closed ty) -> Terminates c
termination ty c with normalization c
termination O c | p = p
termination (ty => ty₁) (Closure t env) | Halts x x₁ , x₂ = Halts x x₁
termination (ty => ty₁) (Clapp c c₁) | x , x₁ = x
