...
-- student: helper func, combine vecs with +. easier solution is ..??
vzip : {n : Nat} {a : Nat} -> Vec Nat n -> Vec Nat n -> Vec Nat n
vzip Nil Nil = Nil
...
madd Nil Nil = Nil
madd (Cons xss xss₁) (Cons yss yss₁) = Cons (vzip xss yss) (madd xss₁ yss₁)


-- Define the identity matrix
-- student: use implicit argument to construct it smart.
idMatrix : {n : Nat} -> Matrix Nat n n
idMatrix {Zero} = Nil
idMatrix {Succ n} = {!!}

-- student: helper, for transpose
tail : {a : Set} {n : Nat} -> Vec a (Succ n) -> Vec a n
tail (Cons x xs) = xs

-- Define matrix transposition
-- student: something something 'vmap head' & 'vmap tail' ...
transpose : {n m : Nat} {a : Set} -> Matrix a m n -> Matrix a n m
transpose Nil = {!!}
transpose (Cons mat mat₁) = {!!}
...
-- student: note: use cong & embed & refl
...
plusZero Zero = Refl
plusZero (Succ num) = cong Succ (plusZero num)

plusSucc : (n m : Nat) -> Succ (n + m) == (n + Succ m)
plusSucc Zero m = Refl
plusSucc n Zero = {!!}
plusSucc (Succ n) (Succ m) = {!!}

plusCommutes : (n m : Nat) -> (n + m) == (m + n)
plusCommutes Zero Zero = Refl
plusCommutes Zero (Succ m) = {!!}
plusCommutes (Succ n) Zero = {!!}
...
SubListTrans (Keep xs) (Drop ys) = Drop {!!}
SubListTrans (Drop xs) (Keep ys) = Drop (SubListTrans xs ys)
SubListTrans (Drop xs) (Drop ys) = Drop {!!}

SubListAntiSym : {a : Set} {xs ys : List a} ->  SubList xs ys -> SubList ys xs -> xs == ys
SubListAntiSym Base sl2 = Refl
SubListAntiSym (Keep sl1) (Keep sl2) = {!!}
SubListAntiSym (Keep sl1) (Drop sl2) = {!!}
SubListAntiSym (Drop sl1) (Keep sl2) = {!!}
SubListAntiSym (Drop sl1) (Drop sl2) = {!!}
