...
\usepackage{listings}
\lstnewenvironment{code}{\lstset{language=Agda,basicstyle=\small}}{}
...
I have added the new module (Regular2Multirec) to the bottom of this file. I have also added a
few methods (map∘, map∀ and mapId) to the Multirec module, which is why I edited the original file
...
module Regular where
  ...
  ⟦ F ⊕ G  ⟧  A = ⟦ F ⟧ A  ⊎  ⟦ G ⟧ A
  ⟦ F ⊗ G  ⟧  A = ⟦ F ⟧ A  ×  ⟦ G ⟧ A
  data μ (F : Code) : Set where ⟨_⟩ : ⟦ F ⟧ (μ F) → μ F
...
module Multirec where
  ...
  ⟦ F ⊕ G  ⟧  r i = ⟦ F ⟧ r i  ⊎  ⟦ G ⟧ r i
  ⟦ F ⊗ G  ⟧  r i = ⟦ F ⟧ r i  ×  ⟦ G ⟧ r i
  data μ {I : Set} (F : Code I) (i : I) : Set where ⟨_⟩ : ⟦ F ⟧ (μ F) i → μ F i
...
\begin{code}
  map∘ : {I : Set} {A B C : Indexed I} (D : Code I)
         ...
         → map D f i (map D g i x) ≡ map D (λ i → (f i) ∘ (g i)) i x
  map∘ U = refl
  map∘ (I' x) = refl
  map∘ (! x) = refl
  ...
  map∀ : {I : Set} (C : Code I) {A B : Indexed I} {f g : ∀ i → A i → B i} → (∀ i x → f i x ≡ g i x) → (∀ i x → map C f i x ≡ map C g i x)
  map∀ U p x x₁ = refl
  map∀ (I' j) p j' x₁ = p j x₁
  map∀ (! j) p j' x₁ = refl
  ...
  mapId : {I : Set} {A : Indexed I} (C : Code I) → {i : I} → {x : ⟦ C ⟧ A i} → map {I} {A} {A} C (λ i → id) i x ≡ x
  mapId U = refl
  mapId (I' _) = refl
  mapId (! _) = refl
...
module Regular2PolyP where
  ...
  fromμRegular C (Regular.⟨_⟩ x) = (PolyP.⟨_⟩ (fromRegular C (Regular.map C (fromμRegular C) x)))
...
module Regular2Multirex where
  ...
  r2mrC (c₁ Regular.⊗ c₂) = (r2mrC c₁) Multirec.⊗ (r2mrC c₂)
  -- Note: the ! constructor of multirec is just not used at all, because we don't need the "type checking"
  -- Every regular code is transformed into a multirec code with an index from ⊤ (with only one element, namely tt)
  -- This represents the fact that indices are not really relevant in this case.

  -- STEP 2. Define the isomorphism between the interpretations in both systems, and their fixed points (μ).
  -- This consists of two directions, "from" (Regular -> Multirec) and "to" (Multirec -> Regular)
  fromRegular : {R : Set} → (C : Regular.Code) → (Regular.⟦_⟧ C R) → ((Multirec.⟦_⟧ (r2mrC C) (λ X → R)) tt)
  fromRegular Regular.U               tt = tt
  fromRegular Regular.I'               x = x
  ...
  fromμRegular : (C : Regular.Code) → Regular.μ C → Multirec.μ (r2mrC C) tt
  fromμRegular c Regular.⟨ x ⟩ = Multirec.⟨_⟩ (fromRegular c (Regular.map c (fromμRegular c) x))

  toRegular : {R : Set} → (C : Regular.Code) → ((Multirec.⟦_⟧ (r2mrC C) (λ X → R)) tt) → Regular.⟦_⟧ C R 
  toRegular Regular.U = id
  toRegular Regular.I' = id
  toRegular (c₁ Regular.⊕ c₂) = [_,_] (inj₁ ∘ (toRegular c₁)) (inj₂ ∘ (toRegular c₂))
  toRegular (c₁ Regular.⊗ c₂) = <_,_> ((toRegular c₁) ∘ proj₁) ((toRegular c₂) ∘ proj₂)

  toμRegular : (C : Regular.Code) → Multirec.μ (r2mrC C) tt → Regular.μ C
  toμRegular c Multirec.⟨ x ⟩ = Regular.⟨_⟩ (toRegular c (Multirec.map (r2mrC c) (λ X → toμRegular c) tt x))


  -- STEP 3. Show that the functions just defined indeed do form an isomorphism. This again consists of two directions:
  -- show that to ∘ from = id, and that from ∘ to = id. Again, both for the regular case and for the fixed point.
  -- STEP 3a. to ∘ from = id
  iso₁ : {R : Set} → (C : Regular.Code) → (x : Regular.⟦_⟧ C R) → toRegular C (fromRegular C x) ≡ x
  ...
  -- helper's lemma for isoμ₁. I could have written R₁ R₂ : Set, because they are transformed into a set immediately
  -- by giving tt as an argument. But I think this with this definition the structure of the type is more clear.
  lemma₁ : {R₁ R₂ : Multirec.Indexed ⊤} (C : Regular.Code)
           {f : (R₁ tt) → (R₂ tt)} (x : ((Multirec.⟦_⟧ (r2mrC C) (λ X → (R₁ tt))) tt))
         → toRegular C (Multirec.map (r2mrC C) (λ i → f) tt x) ≡ Regular.map C f (toRegular C x)
  lemma₁ Regular.U _                = refl
  lemma₁ Regular.I' _               = refl
  ...
  isoμ₁ c Regular.⟨ x ⟩ = cong Regular.⟨_⟩ $
       begin
          toRegular c (Multirec.map (r2mrC c) (λ i → toμRegular c) tt (fromRegular c (Regular.map c (fromμRegular c) x)))
       ≡⟨ lemma₁ c _ ⟩ -- take map outside
          Regular.map c (toμRegular c) (toRegular c (fromRegular c (Regular.map c (fromμRegular c) x)))
       ≡⟨ cong (Regular.map c (toμRegular c)) (iso₁ c _)⟩ --use regular iso₁ to make inside of the equation smaller
          Regular.map c (toμRegular c) (Regular.map c (fromμRegular c) x)
       ≡⟨ Regular.map∘ c ⟩ -- composition of two maps (map the composition of the two functions at the same time)
          Regular.map c (toμRegular c ∘ fromμRegular c) x
       ≡⟨ Regular.map∀ c (isoμ₁ c) x ⟩ -- recursion step (toμRegular ∘ fromμRegular = id, for x)
          Regular.map c id x
       ≡⟨ Regular.mapId c ⟩ -- map id does nothing
          x ∎

  -- STEP 3b. from ∘ to = id. Similar to 3a, but had to define map∘, map∀, and mapId in the Multirec module for this.
  -- They were already defined in the Regular module.
  iso₂ : {R : Set} → (C : Regular.Code) → (x : (Multirec.⟦_⟧ (r2mrC C) (λ X → R)) tt) → fromRegular C (toRegular C x) ≡ x
  iso₂ Regular.U tt               = refl
  iso₂ Regular.I' _               = refl
  iso₂ (c₁ Regular.⊕ c₂) (inj₁ x) = cong inj₁ (iso₂ c₁ x)
  iso₂ (c₁ Regular.⊕ c₂) (inj₂ y) = cong inj₂ (iso₂ c₂ y)
  ...
  -- helper's lemma for isoμ₂.
  lemma₂ : {R₁ R₂ : Multirec.Indexed ⊤} (C : Regular.Code)
           {f : (R₁ tt) → (R₂ tt)} (x : Regular.⟦_⟧ C (R₁ tt))
         → fromRegular C (Regular.map C f x) ≡ Multirec.map (r2mrC C) (λ i → f) tt (fromRegular C x)
  ...
  isoμ₂ : (C : Regular.Code) → (x : Multirec.μ (r2mrC C) tt) → fromμRegular C (toμRegular C x) ≡ x
  isoμ₂ c Multirec.⟨ x ⟩ = cong Multirec.⟨_⟩ $
     begin
        fromRegular c (Regular.map c (fromμRegular c) (toRegular c (Multirec.map (r2mrC c) (λ i → toμRegular c) tt x)))
     ≡⟨ lemma₂ c _ ⟩
        Multirec.map (r2mrC c) (λ i → fromμRegular c) tt (fromRegular c (toRegular c (Multirec.map (r2mrC c) (λ i → toμRegular c) tt x)))
     ≡⟨ cong (Multirec.map (r2mrC c) (λ i → fromμRegular c) tt) (iso₂ c _) ⟩
        Multirec.map (r2mrC c) (λ i → fromμRegular c) tt (Multirec.map (r2mrC c) (λ i → toμRegular c) tt x)
     ≡⟨ Multirec.map∘ (r2mrC c) ⟩
        Multirec.map (r2mrC c) (λ i → (fromμRegular c ∘ toμRegular c)) tt x
     ≡⟨ Multirec.map∀ (r2mrC c) (λ i → isoμ₂ c) tt x ⟩
        Multirec.map (r2mrC c) (λ i → id) tt x
