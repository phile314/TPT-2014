...
data _⇓_ : {ty : Type} -> Term ty → Value ty → Set where
  ...
  EvalIfTrue : forall {c ty} {v : Value ty} {t e : Term ty} -> c ⇓ vtrue -> t ⇓ v -> if c then t else e ⇓ v
  EvalIfFalse : forall {c ty} {v : Value ty} {t e : Term ty} -> c ⇓ vfalse -> e ⇓ v -> if c then t else e ⇓ v
  EvalZero : zero ⇓ vnat Zero
  EvalSucc : {n : Nat} {tt : Term NAT} -> tt ⇓ vnat n -> succ tt ⇓ vnat (Succ n)
  EvalIsZeroZero : {t : Term NAT} -> t ⇓ vnat Zero -> iszero t ⇓ vtrue
  EvalIsZeroSucc : {n : Nat} {t : Term NAT} -> t ⇓ vnat (Succ n) -> iszero t ⇓ vfalse
...
big-to-small (EvalIfTrue c f) = E-If-Steps (big-to-small c) ++ [ E-If-True ] ++ big-to-small f 
big-to-small (EvalIfFalse c f) = E-If-Steps (big-to-small c) ++ [ E-If-False ] ++ big-to-small f
big-to-small EvalZero = Nil
big-to-small (EvalSucc a) = E-Succ-Steps (big-to-small a)
big-to-small (EvalIsZeroZero a ) = E-IsZero-Steps (big-to-small a) ++ [ E-IsZeroZero ]
big-to-small (EvalIsZeroSucc {n} a) = E-IsZero-Steps (big-to-small a) ++ [ E-IsZeroSucc {vnat n} ]
...
prepend-step E-If-True t = EvalIfTrue EvalTrue t
prepend-step E-If-False t = EvalIfFalse EvalFalse t
prepend-step (E-If-If s) (EvalIfTrue t t₁) = EvalIfTrue (prepend-step s t) t₁
prepend-step (E-If-If s) (EvalIfFalse t t₁) = EvalIfFalse (prepend-step s t) t₁
prepend-step (E-Succ s) (EvalSucc t) = EvalSucc (prepend-step s t)
...
prepend-step (E-IsZero s) (EvalIsZeroZero t) = EvalIsZeroZero (prepend-step s t)
prepend-step (E-IsZero s) (EvalIsZeroSucc t) = EvalIsZeroSucc (prepend-step s t)

small-to-big : {ty : Type} -> (t : Term ty) -> (v : Value ty) → Steps t ⌜ v ⌝ → t ⇓ v
small-to-big .(⌜ v ⌝) v Nil = value-to-value v
small-to-big t v (Cons x s) = prepend-step x (small-to-big _ v s)
...
... | a with ⟦ t ⟧
⇓-complete (if t then t₁ else t₂) | a | vtrue = EvalIfTrue a (⇓-complete t₁)
⇓-complete (if t then t₁ else t₂) | a | vfalse = EvalIfFalse a (⇓-complete t₂)
⇓-complete zero = EvalZero
⇓-complete (succ t) with ⇓-complete t
... | a with ⟦ t ⟧
⇓-complete (succ t) | a | vnat x = EvalSucc a
⇓-complete (iszero t) with ⇓-complete t
... | a with ⟦ t ⟧
⇓-complete (iszero t) | a | vnat Zero = EvalIsZeroZero a
⇓-complete (iszero t) | a | vnat (Succ x) = EvalIsZeroSucc a
...
⇓-sound (EvalIfTrue {t} p p2) with ⟦ t ⟧ | ⇓-sound p
⇓-sound (EvalIfTrue p p2) | vtrue | refl = ⇓-sound p2
⇓-sound (EvalIfFalse {t} p p₁) with ⟦ t ⟧ | ⇓-sound p
⇓-sound (EvalIfFalse p p₁) | vfalse | refl = ⇓-sound p₁
⇓-sound EvalZero = refl
⇓-sound (EvalSucc {t} {v} p) with ⟦ v ⟧ | ⇓-sound p
⇓-sound (EvalSucc p) | vnat n | refl = refl
⇓-sound (EvalIsZeroZero {v} p) with ⟦ v ⟧ | ⇓-sound p
⇓-sound (EvalIsZeroZero p) | vnat .Zero | refl = refl
⇓-sound (EvalIsZeroSucc {t} {v} p) with ⟦ v ⟧ | ⇓-sound p
⇓-sound (EvalIsZeroSucc p) | vnat ._ | refl = refl
