...
-- Implementation of Regular 2 Multirec
-- Used Regular2PolyP as starting point / base
-- and adjusted it to work with Multirec instead.
module Regular2Multirec where
  r2mC : Regular.Code → Multirec.Code ⊤
  r2mC Regular.U = Multirec.U
  r2mC Regular.I' = Multirec.I' tt
  r2mC (f Regular.⊕ g) = (r2mC f) Multirec.⊕ (r2mC g)
  r2mC (f Regular.⊗ g) = (r2mC f) Multirec.⊗ (r2mC g)

  fromRegular : {R : Set} (C : Regular.Code) → (Regular.⟦_⟧ C R) → (Multirec.⟦_⟧ (r2mC C) (const R) tt)
  fromRegular Regular.U e = tt
  fromRegular Regular.I' e = e
  fromRegular (f Regular.⊕ g) (inj₁ x) = inj₁ (fromRegular f x)
  fromRegular (f Regular.⊕ g) (inj₂ y) = inj₂ (fromRegular g y)
  fromRegular (f Regular.⊗ g) (x , y) = fromRegular f x , fromRegular g y

  toRegular : {R : Set} (C : Regular.Code) → Multirec.⟦_⟧ (r2mC C) (const R) tt → Regular.⟦_⟧ C R
  toRegular Regular.U = id
  toRegular Regular.I' = id
  toRegular (f Regular.⊕ g) = [_,_] (inj₁ ∘ toRegular f) (inj₂  ∘ toRegular g)
  toRegular (f Regular.⊗ g) = <_,_> (toRegular f  ∘  proj₁) (toRegular g ∘ proj₂)
  ...
  toμRegular c Multirec.⟨ x ⟩ = Regular.⟨ toRegular c (Multirec.map (r2mC c) (const (toμRegular c)) tt x) ⟩
  ...
  lemma₁ : {R₁ R₂ : Set} (C : Regular.Code) {f : R₁ → R₂} (x : Multirec.⟦_⟧ (r2mC C) (const R₁) tt)
               → toRegular C (Multirec.map (r2mC C) (const f) tt x) ≡ Regular.map C f (toRegular C x)
  lemma₁ Regular.U _ = refl
  lemma₁ Regular.I' _ = refl
  lemma₁ (f Regular.⊕ g) (inj₁ x) = cong inj₁ (lemma₁ f x)
  lemma₁ (f Regular.⊕ g) (inj₂ y) = cong inj₂ (lemma₁ g y)
  ...
  isoμ₁ c Regular.⟨ x ⟩ = cong Regular.⟨_⟩ $
      ...
      begin
        ...
        toRegular c (Multirec.map (r2mC c) (const (toμRegular c)) tt (fromRegular c (Regular.map c (fromμRegular c) x)))
