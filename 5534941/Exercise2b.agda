...
normalizableStep {O} a (Halts x x₁) = Halts x (Cons a x₁)
normalizableStep {ty => ty₁} {c1} {c2} a (Halts x x₁ , x₂) = Halts x (Cons a x₁) , (λ p q → normalizableStep (AppL c1 c2 p a) (x₂ p q))
...
-- Sort of taken from 2a exercise
values-are-normal-forms : ∀ {ty} → {c : Closed ty} → IsValue c → NF c
values-are-normal-forms {c = Closure (Abs t) env} a (Red ())
values-are-normal-forms {c = Closure (App t t₁) env} a x = a
values-are-normal-forms {c = Closure (Var x) env} a x₁ = a
values-are-normal-forms {c = Clapp c c₁} a x = a
...
mutual
  ...
  clapp-normalization {c2 = c2} (a , b) c = b c2 c
  ...
  closure-normalization ._ (Abs t) env n = Halts (values-are-normal-forms U) Nil , (λ p q → normalizableStep (Beta t p) (closure-normalization _ t (Cons p env) (q , n)))
  closure-normalization ty (App t t₁) env n = normalizableStep Dist (clapp-normalization (closure-normalization _ t env n) (closure-normalization _ t₁ env n))
  closure-normalization ty (Var x) env n = normalizableStep Lookup (lemma env x n)
    where
    lemma : ∀ {ctx}{t : Type} → (env : Env ctx) → (x : Ref ctx t) → NormalizableEnv env  → Normalizable t (lookup x env)
    lemma (Cons x₁ env₁) Top (x₂ , x₃) = x₂
    lemma (Cons x₁ env₁) (Pop x₂) (x₃ , x₄) = lemma env₁ x₂ x₄
  ...
  normalizable-env {env = Cons x env} = normalization _ x , normalizable-env {env = env}
