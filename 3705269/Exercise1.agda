...
pure {Zero}     a1 = Nil
pure {Succ n}   a1 = Cons a1 (pure {n} a1)

-- We only need to match on the first argument as Nil, as this implies a Nil in the second argument
...
madd xss yss = ? {-where
    pair : {n : Nat} -> Vec Nat n -> Vec Nat n -> Pair (Vec Nat n) (Vec Nat n)
    pair a b = (a , b)
    -- Create a vector of addition functions
    f : {n : Nat} -> Vec Nat n -> Vec (Nat -> Nat) n
    f xs = vmap (\x y -> x + y) xs

    fxss = vmap (\x -> f x) xss

    g : {n : Nat} -> Vec (Nat -> Nat) n -> Vec Nat n -> Vec Nat n
    g xs ys = vmap (\x -> x <*> xs) ys-}
...
plan {Zero} = Nil
--plan {Succ n} = Cons (Succ n) (plan {n}) -- TODO: does not work
plan {Succ n} = ?
...
--cmp (Succ n) (Succ m)   = cmp n m -- TODO: does not work
cmp (Succ n) (Succ m)   = {!!}
...
difference n m = diff (cmp n m) where
    -- Reduces the arguments until one of them equals zero
    -- this function alone would be enough, but we have to use
    -- the cmp function...
    -- We use diff to make the first argument the largest of the two
    reduce : (n m : Nat) -> Nat
    reduce Zero b               = b
    reduce a Zero               = a -- Will never be reached, when diff is used
    reduce (Succ a) (Succ b)    = reduce a b

    -- Gives the correct arguments to 'reduce', or results Zero
    diff : {n m : Nat} -> Compare n m -> Nat
    diff (LessThan {a} b)       = reduce a b
    diff (Equal)                = Zero
    diff (GreaterThan {a} b)    = reduce b a
...
plusZero Zero       = Refl
plusZero (Succ n)   = cong Succ (plusZero n) where

plusSucc : (n m : Nat) -> Succ (n + m) == (n + Succ m)
plusSucc Zero       Zero        = Refl
plusSucc (Succ n)   Zero        = cong Succ (plusSucc n Zero)
plusSucc Zero       (Succ m)    = cong Succ (plusSucc Zero m)
plusSucc (Succ n)   (Succ m)    = ? --cong Succ (plusSucc n m)
...
plusCommutes (Succ n) (Succ m) = ? -- cong Succ (plusCommutes n m)
