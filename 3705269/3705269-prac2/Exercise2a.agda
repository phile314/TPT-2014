...
data _⇓_ : {ty : Type} -> Term ty → Value ty → Set where
  ...
  EvalSucc : forall {n k} -> n ⇓ vnat k -> succ n ⇓ vnat (Succ k)
  EvalIsZeroZero : {n : Term NAT} -> n ⇓ vnat Zero -> iszero n ⇓ vtrue
  EvalIsZeroSucc : forall {n m} -> n ⇓ vnat (Succ m) -> iszero n ⇓ vfalse
...
big-to-small (EvalIsZeroSucc p) = E-IsZero-Steps (big-to-small p) ++ [ E-IsZeroSucc ]
big-to-small (EvalIfTrue p p1) = E-If-Steps (big-to-small p) ++ [ E-If-True ] ++ big-to-small p1
big-to-small (EvalIfFalse p p1) = E-If-Steps (big-to-small p) ++ [ E-If-False ] ++ big-to-small p1
...
prepend-step E-If-True e = EvalIfTrue EvalTrue e
prepend-step E-If-False e = EvalIfFalse EvalFalse e
prepend-step (E-If-If s) e = {!   !}
prepend-step (E-Succ s) e = {! EvalSucc (prepend-step s e) e !}
prepend-step E-IsZeroZero e = {!   !}
prepend-step E-IsZeroSucc e = {!   !}
prepend-step (E-IsZero s) e = {!   !}

small-to-big : {ty : Type} -> (t : Term ty) -> (v : Value ty) → Steps t ⌜ v ⌝ → t ⇓ v
small-to-big .(⌜ v ⌝) v Nil = value-to-value v
small-to-big t v (Cons x s) = {!prepend-step x !}
