...
module Regular2Multirec where
  ...
  -- Convert the Code in regular to code in Multirec
  regularCode2multirecCode : Regular.Code → Multirec.Code ⊤
  regularCode2multirecCode Regular.U = Multirec.U
  regularCode2multirecCode Regular.I' = Multirec.I' tt 
  regularCode2multirecCode (x Regular.⊕ x₁) = regularCode2multirecCode x Multirec.⊕ regularCode2multirecCode x₁
  regularCode2multirecCode (x Regular.⊗ x₁) = (regularCode2multirecCode x) Multirec.⊗ (regularCode2multirecCode x₁)

  -- Shorthand for regular semantics
  r⟦_⟧ : Regular.Code → Set → Set
  r⟦_⟧ = Regular.⟦_⟧

  -- Shorthand for multirec semantics
  m⟦_⟧ : {I : Set} → Multirec.Code I → Multirec.Indexed I → I → Set
  m⟦_⟧ = Multirec.⟦_⟧

  -- Convert a representation in regular to a representation in multirec
  fromRegular : {R : Set} (C : Regular.Code) → (r⟦ C ⟧ R) → (m⟦ regularCode2multirecCode C ⟧ (λ _ → R) tt)
  ...
  toRegular : {R : Set} (C : Regular.Code) → m⟦ regularCode2multirecCode C ⟧ (λ _ → R) tt → r⟦ C ⟧ R
  toRegular Regular.U m = tt
  toRegular Regular.I' m = m
  ...
  fromμRegular : (C : Regular.Code) → Regular.μ C → Multirec.μ (regularCode2multirecCode C) tt
  fromμRegular c Regular.⟨ x ⟩ = Multirec.⟨ (fromRegular c (Regular.map c (fromμRegular c) x)) ⟩

  toμRegular : (C : Regular.Code) → Multirec.μ (regularCode2multirecCode C) tt → Regular.μ C
  toμRegular c Multirec.⟨ x ⟩ = Regular.⟨ (toRegular c (Multirec.map (regularCode2multirecCode c) ((λ _ → toμRegular c)) tt x)) ⟩
