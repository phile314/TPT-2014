...
vapp : {n : Nat}{a b c : Set} -> (a -> b -> c) -> Vec a n -> Vec b n -> Vec c n --
vapp f v w = vmap (λ x y → f x y) v <*> w --

madd : {n m : Nat} -> Matrix Nat m n -> Matrix Nat m n -> Matrix Nat m n
madd xss yss = vapp (vapp (λ x y → x + y)) xss yss

-- Define the identity matrix
idMatrix : {n : Nat} -> Matrix Nat n n
idMatrix {n} = {!!}

nullVec : {n : Nat} -> Vec Nat n
nullVec {Zero} = Nil
nullVec {Succ a} = Cons 0 (nullVec {a})

--idVec {Succ (Succ (Succ 0))} (Fs Fz)
idVec : {n : Nat} -> (m : Fin n) -> Vec Nat n
idVec {Zero} ()
idVec {Succ a} Fz = Cons (Succ 0) nullVec
idVec {Succ a} (Fs m) = Cons 0 (idVec m)

naturals : {n : Nat} -> Vec Nat n
naturals {Zero} = Nil
naturals {Succ a} = Cons (a) (naturals {a})
...
foldl : {a b : Set}{n : Nat} -> (b -> a -> b) -> b -> Vec a n -> b
foldl op init Nil = init
foldl op init (Cons x xs) = foldl op (op init x) xs

_-_ : (n : Nat) (m : Fin  n) -> Nat
Zero - ()
Succ a - Fz = Succ a
Succ a - Fs b = a - b
-}
...
embed (Fz {n}) = Fz {Succ n}
embed (Fs {n} a) = Fs {Succ n} (embed a)
...
plusZero Zero = Refl
plusZero (Succ i) = cong Succ (plusZero i)
