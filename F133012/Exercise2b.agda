...
-- Having a step from t₁ → t₂, and knowing that t₂ terminates implies
-- that t₁ terminates too!
terminates-trans : ∀{ty}{c1 c2 : Closed ty} → Terminates c2 → Step c1 c2 → Terminates c1
terminates-trans (Halts x x₁) c1c2 = Halts x (Cons c1c2 x₁)
...
normalizableStep {O} c1c2 nc2              = terminates-trans nc2 c1c2
normalizableStep {ty => ty₁} {c1} {c2} c1c2 (x , x₁) 
   = terminates-trans x c1c2 
   , (λ x₂ x₃ → normalizableStep (AppL c1 c2 x₂ c1c2) (x₁ x₂ x₃))
...
norm-env⇒lkup-term : ∀{ctx}(ty : Type)(env : Env ctx)(x : Ref ctx ty)
                   → NormalizableEnv env → Normalizable ty (lookup x env)
norm-env⇒lkup-term ty (Cons x env) Top nenv = fst nenv
norm-env⇒lkup-term ty (Cons x env) (Pop ref) nenv = norm-env⇒lkup-term ty env ref (snd nenv)
...
mutual
  ...
  clapp-normalization {c2 = c2} (x , x₁) nc2 = x₁ c2 nc2
  ...
  closure-normalization ty (App {u = u} {v = .ty} m n) env nenv 
    with closure-normalization (u => ty) m env nenv
       | closure-normalization u n env nenv
  ...| _ , hip-m | hip-n = normalizableStep Dist (hip-m (Closure n env) hip-n)
  closure-normalization ty (Var x) env nenv 
    = normalizableStep Lookup (norm-env⇒lkup-term ty env x nenv)
  closure-normalization ._ (Abs {u = u} {v = v} t) env nenv
    = (Halts (λ { (Red ()) }) Nil) 
    , (λ x x₁ → normalizableStep (Beta t x) (closure-normalization v t (Cons x env) (x₁ , nenv)))
  ...
  normalizable-env {ctx = c :: ctx} {env = Cons x env} 
    = normalization c x , normalizable-env
  ...
  normalization ty (Clapp {ty = ty'} c c₁)
    with normalization (ty' => ty) c | normalization ty' c₁
  ...| nc | nc1 = clapp-normalization nc nc1
