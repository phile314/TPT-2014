...
normalizableStep : {ty : Type} {c1 c2 : Closed ty} -> Step c1 c2 ->
   Normalizable ty c2 -> Normalizable ty c1
normalizableStep {O} step (Halts x y) = Halts x (Cons step y)
normalizableStep {ty => ty'} step (Halts x y , z) = Halts x (Cons step y) , (λ p q → normalizableStep (AppL _ _ p step) (z p q))
...
mutual
  ...
  clapp-normalization (x , f) n = f _ n
  ...
  closure-normalization (Abs t) ctx env = {!!} , (λ p q → normalizableStep (Beta t p) (closure-normalization t (Cons p ctx) (q , env)))
  closure-normalization (App t t') ctx env with closure-normalization t ctx env | closure-normalization t' ctx env
  closure-normalization (App t t') ctx env | x , y | q = normalizableStep Dist (y _ q)
  closure-normalization (Var x) ctx₁ env = normalizableStep Lookup {!!}
  ...
  normalizable-env {x :: ctx} {Cons y env} = normalization x y , normalizable-env

  -- All terms are normalizable.
  normalization : (ty : Type) -> (c : Closed ty) -> Normalizable ty c
  normalization ty (Closure t env) = closure-normalization t env normalizable-env
  normalization ty (Clapp c d) = clapp-normalization (normalization (_ => ty) c) (normalization _ d)
