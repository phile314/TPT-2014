...
>  ((((crush (C ⊗ D))  _comb_) frec) zero') (_,_ x y)    = (crush C (_comb_) frec zero' x) comb (crush D _comb_ frec zero' y)
>  size : (A : Code) → ⟦ A ⟧ → ℕ
>  size C = crush C _+_ Data.Nat.suc 0
>  aList : ⟦ ListC ⊤ ⟧
>  aList = inj₂ ((_,_) (k tt) (rec (inj₂ ((_,_) (k tt) (rec (inj₁ tt))))))
>
...
>  r2mR : Regular.Code → Multirec.Code ⊤
>  r2mR Regular.U = Multirec.U
>  r2mR Regular.I' = Multirec.I' tt
>  r2mR (F Regular.⊕ G) = r2mR F Multirec.⊕ r2mR G
>  r2mR (F Regular.⊗ G) = r2mR F Multirec.⊗ r2mR G
>
>  fromRegular : {R : Set} (C : Regular.Code) → (Regular.⟦_⟧ C R) → (Multirec.⟦_⟧ (r2mR C) (λ _ → R) tt)
>  fromRegular Regular.U y = y
>  fromRegular Regular.I' y = y
>  fromRegular (F Regular.⊕ G) (inj₁ x) = inj₁ (fromRegular F x)
>  fromRegular (F Regular.⊕ G) (inj₂ x) = inj₂ (fromRegular G x)
>  fromRegular (F Regular.⊗ G) (x , y) = fromRegular F x , fromRegular G y
>
>  fromμRegular : (C : Regular.Code) → Regular.μ C → Multirec.μ (r2mR C) tt
>  fromμRegular C Regular.⟨ x ⟩ = Multirec.⟨ fromRegular C (Regular.map C (fromμRegular C) x) ⟩
>
>  toRegular : {R : Set} (C : Regular.Code) → Multirec.⟦_⟧ (r2mR C) (λ _ → R) tt → Regular.⟦_⟧ C R
>  toRegular Regular.U y = tt
>  toRegular Regular.I' y = y
>  toRegular (F Regular.⊕ G) (inj₁ x) = inj₁ (toRegular F x)
>  toRegular (F Regular.⊕ G) (inj₂ x) = inj₂ (toRegular G x)
>  toRegular (F Regular.⊗ G) (x , y) = toRegular F x , toRegular G y
>
>  toμRegular : (C : Regular.Code) → Multirec.μ (r2mR C) tt → Regular.μ C
>  toμRegular C Multirec.⟨ x ⟩ = Regular.⟨ toRegular C (Multirec.map (r2mR C) (λ i x₁ → toμRegular C Multirec.⟨ x ⟩) tt x) ⟩
>
>  iso₁ : {R : Set} → (C : Regular.Code) → (x : (Regular.⟦_⟧ C) R) → toRegular C (fromRegular C x) ≡ x
>  iso₁ Regular.U tt = refl
>  iso₁ Regular.I' y = refl
>  iso₁ (F Regular.⊕ G) (inj₁ x) = cong inj₁ (iso₁ F x)
>  iso₁ (F Regular.⊕ G) (inj₂ x) = cong inj₂ (iso₁ G x)
>  iso₁ (F Regular.⊗ G) (x , y) = cong₂ _,_ (iso₁ F x) (iso₁ G y)
>
>  lemma₁ : {R₁ R₂ : Set} (C : Regular.Code)
>           {f : R₁ → R₂} (x : Multirec.⟦_⟧ (r2mR C) (λ _ → R₁) tt)
>         → toRegular C (Multirec.map (r2mR C) (λ i → f) tt x)
>         ≡ Regular.map C f (toRegular C x)
>  lemma₁ Regular.U y = refl
>  lemma₁ Regular.I' y = refl
>  lemma₁ (F Regular.⊕ G) (inj₁ x) = cong inj₁ (lemma₁ F x)
>  lemma₁ (F Regular.⊕ G) (inj₂ x) = cong inj₂ (lemma₁ G x)
>  lemma₁ (F Regular.⊗ G) (x , y) = cong₂ _,_ (lemma₁ F x) (lemma₁ G y)
>
>  open ≡-Reasoning
>
>  isoμ₁ : (C : Regular.Code) (x : Regular.μ C) → toμRegular C (fromμRegular C x) ≡ x
...
>
>      begin
>        -- Due to agda compiler going into an infinite loop or taking a lot of time, I have not been able to finish this part.
>        toRegular C (Multirec.map (r2mR C) (λ _ y → {!!}) tt (fromRegular C (Regular.map C (fromμRegular C) x))) 
>
>      ≡⟨ {!!} ⟩ 
>        
>        {!!}
